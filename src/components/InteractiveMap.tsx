...
const InteractiveMap = () => {
  ...
  const getStaticMapUrl = (center: { lat: number; lng: number }, zoom: number, markers: MonasteryLocation[]) => {
    ...
    const markerString = markers.map(m => `${m.lng},${m.lat}`).join('|');
    
    // Using Mappls (formerly MapmyIndia) API format
    return `${baseUrl}/${MAPPLES_API_KEY}/still_image?center=${center.lng},${center.lat}&zoom=${zoom}&size=800x600&markers=${markerString}&markers_icon=https://maps.mapplsapis.com/images/marker.png`;
  };
